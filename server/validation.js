function minMax(whatVar, describeAs, min, max) {
    ...
    if(whatVar.length < min) return `${describeAs} has to be longer than ${min} characters!`;
    else if(whatVar.length > max) return `${describeAs} has to be less than ${max} characters!`;
    else return true;
}
