...
function insert(name, hash) {
  return new Promise((resolve, reject) => {
    db.queryPromise(
      "INSERT INTO user(user_name, user_hash, user_created) VALUES(?, ?, ?);",
...
function resetAttempts(username) {
  ...
  return db.queryPromise(
    "UPDATE user SET user_attempts = 0, user_updated = ? WHERE user_name = ?",
...
function addToLoginAttempt(username) {
  ...
  return db.queryPromise(
    "UPDATE user SET user_attempts = user_attempts + 1, user_updated = ? WHERE user_name = ?",
    [new Date().getTime(), username]
  );
}
...
function searchLike(toSearch, skip, limit) {
  return new Promise((resolve, reject) => {
    ...
    db.queryPromise(
      "SELECT user_name, user_id, user_created FROM user WHERE user_name LIKE ? LIMIT ? OFFSET ?",
